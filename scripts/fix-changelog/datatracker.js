...
const svnMapPath = 'svn-history/datatracker-revmap.txt' // Path to changelog in remote repository, set to null to disable SVN Changeset linking
const coverageLocalPath = './data/release-coverage.json' // Path to coverage json on LOCAL DISK, set to null to disable coverage

// ---------------------------------

const reChunk = /^ {1,2}-- ([\w ]+) <([\w@.-]+)> +([0-9a-zA-Z+: ]+) *$/gm
...
try {
  ...
  for (const match of chlogRaw.matchAll(reChunk)) {
    ...
    entries.push({
      ...
      author: match[1],
      authorEmail: match[2],
      date: DateTime.fromRFC2822(match[3]).setZone('utc'),
      description,
      body
    })
